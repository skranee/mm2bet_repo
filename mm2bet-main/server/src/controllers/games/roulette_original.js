...
const getWinningColor = async winningMultiplier => {
  return new Promise((resolve, reject) => {
    if (winningMultiplier === 2) {
      ...
      resolve("yellow");
    } else if (winningMultiplier === 0) { //0 normally
      resolve("mystery");
    } else {
      reject(
...
const listen = io => {
  ...
  const endCurrentRouletteGame = async () => {
    ...
    try {
      ...
      const timeout = setTimeout(async () => {
        ...
        // If it was a Mystery, generate another round picking multiplier
        if (winningMultiplier === 0) {
            ...
            const timeout = setTimeout(async () => {
              ...
              for (let index = 0; index < GAME_STATE.players.length; index++) {
                ...
                if (player.color === secondWinningColor) {
                  ...
                  const wonAmount = profit; //player.betAmount + profit;
          ...
          for (let index = 0; index < GAME_STATE.players.length; index++) {
            ...
            if (player.color === winningColor) {
              ...
              const wonAmount = profit; //player.betAmount +;
  ...
  io.of("/roulette").on("connection", socket => {
    ...
    socket.on("join-game", async (color, betAmount) => {
      ...
      if (
        typeof color !== "string" ||
        !["blue", "green", "purple", "yellow"].includes(color)
      )
